...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
  ...
  for x in train-clean-100; do
    python examples/fsdp2/cosyvoice3/preprocess/extract_embedding.py --dir data/$x \
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
  ...
  for x in train-clean-100; do
    python examples/fsdp2/cosyvoice3/preprocess/extract_speech_token.py --dir data/$x \
...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
  ...
  for x in train-clean-100; do
    ...
    python examples/fsdp2/cosyvoice3/preprocess/make_parquet_list.py --num_utts_per_parquet 1000 \
      --num_processes 10 \
      --instruct \
      --src_dir data/$x \
