...
class BasicTransformerBlock(nn.Module):
    def __init__(
        ...
        if positional_embeddings and (num_positional_embeddings is None):
            raise ValueError("num_positional_embeddings must be defined when positional_embeddings is set")
    ...
    def forward(
        ...
        for idx, block in enumerate(self.transformer_blocks):
            if idx % 2 == 1 and self.config.interleave_self_attention:
                hidden_states = block(hidden_states, attention_mask=None, encoder_hidden_states=None, temb=temb)
            else:
                hidden_states = block(
                    hidden_states,
                    attention_mask=None,
