...
class MMPluginMixin:
    ...
    def _preprocess_image(
        ...
        r"""
        对单张图片进行预处理，确保其像素数量在指定范围内并转换为 RGB 模式。
        
        步骤：
        1. 若图片总像素超过 image_max_pixels，则按面积比例缩小。
        2. 若图片总像素低于 image_min_pixels，则按面积比例放大。
        3. 非 RGB 模式（如 L、RGBA 等）统一转成 RGB。
        4. 若 image 对象为磁盘文件句柄类型（非 PIL.Image.Image），则调用 .copy() 将其完整加载到内存，
           避免 with 上下文关闭文件后再次访问报错。
        """
        # 1. 超大图片缩小
        ...
        # 4. 处理文件句柄类型，防止外部关闭文件后失效
        # 原注释已详细说明原因：with 语句关闭文件后，PIL.xxxImagePlugin 对象无法再次读取，
        # 通过 .copy() 得到独立的 PIL.Image.Image 对象，彻底脱离文件依赖。
    ...
    def _get_video_sample_indices(
        ...
        r"""
        根据目标帧率 video_fps 与最大采样帧数 video_maxlen，计算需要解码的视频帧索引列表。
        
        逻辑：
        - 若视频总帧数为 0（无限流），直接生成 0..video_maxlen-1 的等差索引。
        - 否则，按 video_fps 与视频时长估算采样帧数，并保证在 [1, total_frames, video_maxlen] 范围内，
          再线性插值出帧索引。
        """
        total_frames = video_stream.frames
        if total_frames == 0:  # 无限流视频
            return np.linspace(0, video_maxlen - 1, video_maxlen).astype(np.int32)

        # 计算期望采样帧数：duration(second) * fps
    ...
    def _regularize_images(self, images: List["ImageInput"], **kwargs) -> Dict[str, List["ImageObject"]]:
        r"""
        将多种形式的图片输入（文件路径、字节流、dict、PIL 对象）统一读取并预处理成标准 PIL.Image.Image 列表。
        
        支持输入类型：
        - str / BinaryIO：文件路径或文件句柄
        - bytes：原始字节
        - dict：{"path":..., "bytes":...} 二选一
        - ImageObject：已经是 PIL 对象
        
        所有情况最终都调用 _preprocess_image 进行尺寸与颜色空间规范化。
        """
        results = []
        for image in images:
    ...
    def _regularize_videos(self, videos: List["VideoInput"], **kwargs) -> Dict[str, List[List["ImageObject"]]]:
        r"""
        将视频文件解码成帧列表，并对每帧调用 _regularize_images 进行统一预处理。
        
        步骤：
        1. 使用 av 打开视频，找到视频流。
        2. 调用 _get_video_sample_indices 计算需要采样的帧索引。
        3. 解码指定帧并转成 PIL.Image。
        4. 用 _regularize_images 对帧列表再做一轮图像级预处理（尺寸、颜色空间等）。
        5. 返回 List[List[ImageObject]]，外层为视频，内层为帧。
        """
        results = []
        for video in videos:
