...
class DataCollatorForVLASequence:
    ...
    def __call__(self, features):
        ...
        if "pixel_values" in features[0]:
            pixel_values = [_to_tensor(f["pixel_values"]) for f in features]
            batch["pixel_values"] = torch.cat(pixel_values, dim=0)
        if "image_grid_thw" in features[0]:
            image_grid_thw = [_to_tensor(f["image_grid_thw"]) for f in features]
            batch["image_grid_thw"] = torch.cat(image_grid_thw, dim=0)

        action_key = "action" if "action" in features[0] else "actions"
        if action_key in features[0]:
            actions = [_to_tensor(f[action_key]) for f in features]
            actions = torch.stack(actions)
            if actions.dim() > 3 and actions.size(1) == 1:
                actions = actions.squeeze(1)
            batch["action"] = actions

        state_key = "state" if "state" in features[0] else "proprio"
        if state_key in features[0] and features[0][state_key] is not None:
            states = [_to_tensor(f[state_key]) for f in features]
...
class DataCollatorForQwen2vlVLA(DataCollatorForVLASequence):
    pass
...
DATA_COLLATOR = {
    ...
    "vla_seq": DataCollatorForVLASequence,
    "qwen2vl_vla": DataCollatorForQwen2vlVLA,
